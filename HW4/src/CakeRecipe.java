...
class CakeRecipe {
  ...
  CakeRecipe(double eggs,
      ...
      double sugar,
      double flour) {
    this(eggs, milk, butter, sugar, flour, false);
  }

  // generates a cake method based on an ideal cake recipe
  CakeRecipe(double eggs, double milk, double flour) {
    this(eggs, milk, eggs, flour, flour, false);
  }

  // takes in the flour, eggs and milk as volumes rather than
  // weights and a boolean flag areVolumes
  CakeRecipe(double eggs, double milk, double flour, boolean areVolumes) {
    this(eggs, milk, eggs, flour, flour, areVolumes);
  }
...
class ExamplesCakeRecipe {
  CakeRecipe C1 = new CakeRecipe(8.0, 10.0, 8.0, 18.0, 18.0);
  CakeRecipe C2 = new CakeRecipe(7.5, 30.8, 7.5, 38.3, 38.3);

  CakeRecipe C3 = new CakeRecipe(10.0, 15.0, 25.0, false);
  CakeRecipe C4 = new CakeRecipe(8.0, 2.5, 8.0, true);
  CakeRecipe C5 = new CakeRecipe(10.0, 15.0, 25.0);
 ...
 boolean testConstructor2(Tester t) {
  ...
  boolean testSameRecipe(Tester t) {
    return t.checkExpect(C1.sameRecipe(C2), false)
        && t.checkExpect(C1.sameRecipe(new CakeRecipe(8.0, 10.0, 18.0)), true)
