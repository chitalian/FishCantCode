...
;(defthm lemma1 (implies (and (listp x)(listp y)(subset x y))
;                        (subset x (cons a y))))

(defthm reflexive (implies (listp x) (subset x x)))


(defthm lemma2 (IMPLIES (AND (LISTP Y)(LISTP Z)(LIST X1)
                             (NOT (IN X1 Z))
                             (IN X1 Y))
                        (NOT (SUBSET Y Z))))

(defthm transitive (implies (and (listp x)(listp y)(listp z)(subset x y)
                                 (subset y z))
                            (subset x z)))
